...
function InProgressOrderCard({ order, type }) {
  ...
  return (
    <div className="service container d-flex flex-column gap-4">
      <div className="row gap-lg-0 gap-3 d-flex align-items-center justify-content-between">
        ...
        <div className="col-lg-5 col-12 d-flex flex-column gap-3 ">
          <div className="service-head mini">
            <Link
              to={`/profile/${order?.accepted_request?.user?.id}`}
              className="request-owner-img"
            >
              <img src={order?.accepted_request?.user?.image} alt="service" />
            </Link>
            <div className="title requester-title">
              <Link
                to={`/profile/${order?.accepted_request?.user?.id}`}
                className="owner"
              >
                <span>{order?.accepted_request?.user?.name}</span>
              </Link>
              <StarsList rate={order?.accepted_request?.user?.rate || 0} />
            </div>
          </div>
        </div>
      </div>
